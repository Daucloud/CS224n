...
\begin{parts}
    ...
    \footnotesize % Reduce font size further
    \begin{tabular}{ l | l | l | l}
    Stack & Buffer & New dependency & Transition \\ \hline
    [ROOT] & [I, presented, my, findings, at, the, NLP, conference] &  & Initial Configuration \\
    $[$ROOT, I] & [presented, my, findings, at, the, NLP, conference] &  &  \texttt{SHIFT}  \\
    ...
    \newline
    \ifans{
        $2n$ steps. Since every word will be shifted onto the stack once, and then popped off the stack once, then the algorithm stops.
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \ifans{
            \alns{
                &h_i=\relu\left(\bx\bw_i+b_{i}\right)=\begin{cases}
                    \bx\bw_i+b_{i}=\sum\limits_{j=0}^{dm}\left(x_jw_{ji}+b_i\right) & \text{if } \bx\bw_i+b_{i} > 0\\
                    0 & \text{otherwise}
                \end{cases}\\
                \therefore&\dfrac{\partial h_i}{\partial x_j}=\begin{cases}
                    w_{ji} &\text{if } \bx\bw_i+b_{i} > 0\\
                    0 &\text{otherwise}
                \end{cases}
            }
        }
        ...
        \newline
        \ifans{
            Since all the entries of $\by$ are 0 except for $y_c=1$, we have:
            \alns{
                J \left(\theta\right)=-\log\hat\by_c
            }
            Then we have:
            \alns{
                \dfrac{\partial J}{\partial \hat\by_c}&=-\frac{1}{\hat\by_c}\\
                \dfrac{\partial \hat\by_c}{\partial \bl_i}&=\begin{cases}
                    \hat\by_c-\hat\by_c^2 & \text{if } i=c\\
                    -\hat\by_c\hat\by_i & \text{otherwise}
                \end{cases}=\hat\by_c \left(\by_i-\hat\by_i\right)
            }
        The latter equality holds because $\by_i=1\iff i=c$\\
        So,
        \alns{
                \dfrac{\partial J}{\partial \bl_i}=\dfrac{\partial J}{\partial \hat\by_c}\dfrac{\partial \hat\by_c}{\partial \bl_i}=\hat\by_i-\by_i
        }
        }
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \textbf{Deliverables:}
        \begin{itemize}
            ...
            \newline
            \ifans{
                \textbf{Best UAS on dev set:} 88.27 \\
                \textbf{Best UAS on test set:} 88.91
            }
    \end{itemize}
    \end{enumerate}
...
\newline
\ifans{
    Using POS tags helps the model learn more easily, as words with certain tags are more likely to be heads, while others are more often dependents.
}
