    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                \newline
                \ifans{
                    The momentum term $\bm$ incorporates only a small fraction $(1-\beta_1)$ of the new gradient at each step, while maintaining a rolling average of previous gradients as the major component. This smoothing effect prevents drastic changes in $\btheta$, leading to more stable training.
                }
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                \newline
                \ifans{
                    Parameters with smaller gradient magnitudes will have smaller values in $\bv$, resulting in larger updates since we divide by $\sqrt{\bv}$. This helps learning by allowing parameters that are updated less frequently or with smaller gradients to take larger steps when they do get updated.
                }
            ...
            \subpart[2]
                ...
                \ifans{
                    \alns{
                        \mathbb{E}_{p\text{drop}}\left[\bh_{\text{drop}}\right]_i=\gamma \left(1-p_{\text{drop}}\right)h_i=hi
                    }
                    So $\gamma=1/\left(1-p_{\text{drop}}\right)$
                }
          ...
          \newline
          \ifans{
            Performing dropout during training helps prevent overfitting by forcing the model to learn more robust features that are not reliant on any one unit. During evaluation, we want to use all of the learned features, so we do not apply dropout.
          }
